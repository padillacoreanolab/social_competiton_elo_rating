...
def get_elo_df(dataframe, winner_id_column, loser_id_column, tie_column, tie_flag, k_factor=20):
    """
    ...
    for index, row in copied_dataframe.dropna(subset=winner_id_column).iterrows():
        ...
        total_match_number += 1
        df = pd.DataFrame.from_dict(index_to_elo_rating_and_meta_data, orient="index")
    return df


def get_latest_elo_for_subject(df, subject_id):
    """Get the latest ELO rating for a specific subject ID"""
    subject_rows = df[df["subject_id"] == subject_id]
    if subject_rows.empty:
        return None

    # Get the row with the highest index for this subject
    latest_index = subject_rows.index.max()
    latest_elo = df.loc[latest_index, "updated_elo_rating"]
